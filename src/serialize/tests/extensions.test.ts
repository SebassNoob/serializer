...
describe('Extensions System', () => {
  ...
  describe('Blob-returning extensions', () => {
    test('should handle ImageData-like extension that returns Blob', () => {
      ...
      expect(() => deserialize(serialized, [mockImageDataExtension])).toThrow('Cannot read Blob synchronously');
    });
    ...
    test('should handle ArrayBuffer extension that returns Blob', () => {
      ...
      expect(() => deserialize(serialized, [mockArrayBufferExtension])).toThrow('Cannot read Blob synchronously');
    });
    ...
    test('should handle binary data extension with Blob serialization', () => {
      ...
      // This demonstrates the limitation with Blob-returning extensions
      expect(() => deserialize(serialized, [mockBinaryDataExtension])).toThrow('Cannot read Blob synchronously');
    });
  });
  ...
  describe('Extension priority and conflicts', () => {
    test('should handle extension priority (first extension wins)', () => {
      ...
      const deserialized = deserialize(serialized, [mockFirstDateExtension, mockSecondDateExtension]) as Date;
      expect(deserialized.getTime()).toBe(date.getTime());
    });
    ...
    test('should handle multiple different extension types in same object', () => {
      ...
      const serialized = serialize(complexObj, [mockDateExtension, mockBigIntExtension, mockSymbolExtension]);
      const deserialized = deserialize(serialized, [mockDateExtension, mockBigIntExtension, mockSymbolExtension]) as typeof complexObj;
  ...
  describe('Extension edge cases', () => {
    ...
    test('should handle extension with null/undefined in object', () => {
      ...
      const deserializedNull = deserialize(serializedNull, [mockNullableExtension]) as { value: null | Date };
      expect(deserializedNull.value).toBeNull();
      
      // Test Date case
      const serializedDate = serialize(objWithDate, [mockNullableExtension]);
      const deserializedDate = deserialize(serializedDate, [mockNullableExtension]) as { value: null | Date };
    ...
    test('should handle duplicate extension names', () => {
      expect(() => serialize({}, mockDuplicateExtensions as any)).toThrow('Duplicate extension name found: \'duplicate\'');
    });
  });
