...
export const mockSymbolExtension: SerializationExtension<symbol> = {
  ...
  deserialize: (value: string | Blob) => Symbol.for((value as string).replace('Symbol(', '').replace(')', '')),
...
export const mockImageDataExtension: SerializationExtension<{
  ...
  canHandle: (value: unknown): value is { width: number; height: number; data: Uint8Array } => 
    ...
    'width' in value && 'height' in value && 'data' in value &&
...
export const mockBinaryDataExtension: SerializationExtension<{
  ...
  canHandle: (value: unknown): value is { type: 'binary'; content: Uint8Array; metadata: { name: string; size: number; } } => 
    typeof value === 'object' && value !== null && 
    'type' in value && value.type === 'binary' &&
    'content' in value && value.content instanceof Uint8Array
};
...
export const mockPersonExtension: SerializationExtension<MockPerson> = {
  name: 'person',
  serialize: (value: MockPerson) => JSON.stringify({
    name: value.name,
    age: value.age,
    email: value.email
  }),
...
export const mockComplexExtension: SerializationExtension<MockComplexType> = {
  ...
  deserialize: (value: string | Blob) => {
    ...
    return {
      ...
      parent: parsed.parentId ? { id: parsed.parentId, children: [], parent: undefined } : undefined
    } as MockComplexType;
  },
  canHandle: (value: unknown): value is MockComplexType => 
    typeof value === 'object' && value !== null && 
    'id' in value && 'children' in value && Array.isArray(value.children)
};
...
export const mockSecondDateExtension: SerializationExtension<Date> = {
  ...
  deserialize: (value: string | Blob) => new Date(parseInt((value as string).replace('SECOND:', ''))),
  canHandle: (value: unknown): value is Date => value instanceof Date
};
