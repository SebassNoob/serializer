...
export function _validateExtensions<T extends readonly SerializationExtension<any>[]>(extensions: T): void {
  ...
  for (const extension of extensions) {
    ...
    if (/^(\$|ref|ext)/.test(extension.name)) {
      throw new Error(`Extension name '${extension.name}' cannot start with reserved prefixes: $, ref, or ext`);
    }
